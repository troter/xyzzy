...
update_column (int column, Char c)
{
  if (c == '\n')
    ...
    column = ((column + active_app().default_tab_columns)
              / active_app().default_tab_columns * active_app().default_tab_columns);
  else
    column += char_width (c);
  return column;
}
  ...
  if (size)
    {
      if (c == '\n')
        ...
        column = ((column + active_app().default_tab_columns) / active_app().default_tab_columns
                  * active_app().default_tab_columns
                  + (size - 1) * active_app().default_tab_columns);
      else
...
make_string_from_vector (lisp vector)
{
  ...
  lisp *p = xvector_contents (vector), *pe = p + xvector_length (vector);
  for (; p < pe; p++)
    check_char (*p);
